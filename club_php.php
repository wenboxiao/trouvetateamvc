						...
						if($don = $reqs->fetch()){
							...
							$reqa->execute(array($_POST['Ville']));	
								if($donne = $reqa->fetch()){
									...
									$reqa->closeCursor();
									$req = $bdd->prepare('INSERT INTO clubs(adresse, descriptionclub, id_club, id_utilisateur, mailclub, nomclub, ville_id, id_sport) VALUES(:adresse, :descriptionclub, :id_club, :id_utilisateur, :mailclub, :nomclub, :ville_id, :id_sport)');
									$req->execute(array(
